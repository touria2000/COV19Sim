...
## Compile the model
m_m <- stan_model("C:/Users/Touria/Documents/NovelCovid19_VaccinationModel/fit_with_vaccination/models/model_Moderna.stan")

## sampling 
time.start_fit <- Sys.time()
fit_Moderna <- sampling(m_m, 
                      data = data, 
                      chains = 4, 
                      iter = 2000, 
                      seed=2022)
...
rstan::check_divergences(fit_Moderna)
check_hmc_diagnostics(fit_Moderna)

# Save the model
save(fit_Moderna, file = 'C:/Users/Touria/Documents/NovelCovid19_VaccinationModel/fit_with_vaccination/saved_models/fit_Moderna')
# Load the model
load('C:/Users/Touria/Documents/NovelCovid19_VaccinationModel/fit_with_vaccination/saved_models/fit_Moderna')
...
print(fit_Moderna, pars = pars1)
print(fit_Moderna, pars = pars1, digits = 4, format = "f")

pars2 = c("recovery_time_S", "recovery_time_A", "recovery_time_H", "recovery_time_Q", "phi")
print(fit_Moderna, pars = pars2, digits = 4, format = "f")

pars3 = c("E0", "IA0", "IS0", "IQ0", "IH0", "R0", "D0", "V0")
print(fit_Moderna, pars = pars3)
print(fit_Moderna, pars = pars3, digits = 4, format = "f")

library(bayesplot)
color_scheme_set("mix-brightblue-red")
mcmc_trace(fit_Moderna, pars = "theta_V") 

#### Post analysis
# Extract posterior distributions for parameters and initial conditions
posts_fit <- rstan::extract(fit_Moderna)
posts_fit <- extract(fit_Moderna)
...
smr_pred <- cbind(as.data.frame(summary(fit_Moderna, 
                                        pars = "pred_cases", 
...
smr_y <-  cbind(as.data.frame(summary(fit_Moderna,
                                      pars = par, 
                                      probs = c(0.025, 0.05, 0.1, 0.25, 0.5, 0.75, 0.9, 0.95, 0.975))$summary),
                Date=Date, cases = cases)
...
ggsave("C:/Users/Touria/Documents/NovelCovid19_VaccinationModel/fit_with_vaccination/plots/Figure 11_moderna.pdf", 
       dpi = 600, width = 20, height = 7, units = "cm")

# Plot vaccination rate
#####
posterior1 <- extract(fit_Moderna, inc_warmup = TRUE, permuted = FALSE)
...
##
posterior2 <- as.matrix(fit_Moderna)
...
# now add the title
title <- ggdraw() + 
  draw_label("Moderna COVID-19 vaccine", size = 8, fontface = 'bold', x = 0, hjust = 0) +
  theme(plot.margin = margin(0, 0, 0, 7))
plot_grid(title, plot_row, ncol = 1, rel_heights = c(0.1, 1))

ggsave("C:/Users/Touria/Documents/NovelCovid19_VaccinationModel/fit_with_vaccination/plots/Figure 9.pdf", 
