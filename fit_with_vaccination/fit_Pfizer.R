...
#### Incidence data
newcases_TN <- read.csv(file="C:/Users/Benbrahim/Documents/Touria/R_Project/NovelCovid19_VaccinationModel/data/newcases_TN.csv", 
                        header=TRUE, sep=",")
...
## Compile the model
m_p <- stan_model("C:/Users/Touria/Documents/NovelCovid19_VaccinationModel/fit_with_vaccination/models/model_Pfizer.stan")

## sampling 
time.start_fit <- Sys.time()
fit_Pfizer <- sampling(m_p, 
                      data = data, 
                      chains = 4, 
                      iter = 2000, 
                      seed=2022)
...
rstan::check_divergences(fit_Pfizer)
check_hmc_diagnostics(fit_Pfizer)

# Save the model
save(fit_Pfizer, file = 'C:/Users/Touria/Documents/NovelCovid19_VaccinationModel/fit_with_vaccination/saved_models/fit_Pfizer')
# Load the model
load('C:/Users/Touria/Documents/NovelCovid19_VaccinationModel/fit_with_vaccination/saved_models/fit_Pfizer')
...
print(fit_Pfizer, pars = pars1)
print(fit_Pfizer, pars = pars1, digits = 4, format = "f")
...
print(fit_Pfizer, pars = pars3)
print(fit_Pfizer, pars = pars3, digits = 4, format = "f")

library(bayesplot)
color_scheme_set("mix-brightblue-red")
mcmc_trace(fit_Pfizer, pars = "theta_V") 

#### Post analysis
# Extract posterior distributions for parameters and initial conditions
posts_fit <- rstan::extract(fit_Pfizer)
posts_fit <- extract(fit_Pfizer)
...
smr_pred <- cbind(as.data.frame(summary(fit_Pfizer, 
                                        pars = "pred_cases", 
...
smr_y <-  cbind(as.data.frame(summary(fit_Pfizer,
                                      pars = par, 
                                      probs = c(0.025, 0.05, 0.1, 0.25, 0.5, 0.75, 0.9, 0.95, 0.975))$summary),
                Date=Date, cases = cases)
...
ggsave("C:/Users/Touria/Documents/NovelCovid19_VaccinationModel/fit_with_vaccination/plots/Figure 11.pdf", 
       dpi = 600, width = 20, height = 7, units = "cm")

# Plot vaccination rate
#####
posterior1 <- extract(fit_Pfizer, inc_warmup = TRUE, permuted = FALSE)
...
##
posterior2 <- as.matrix(fit_Pfizer)
...
# now add the title
title <- ggdraw() + 
  draw_label("Pfizer COVID-19 vaccine", size = 8, fontface = 'bold', x = 0, hjust = 0) +
  theme(plot.margin = margin(0, 0, 0, 7))
plot_grid(title, plot_row, ncol = 1, rel_heights = c(0.1, 1))

ggsave("C:/Users/Benbrahim/Documents/Touria/R_Project/NovelCovid19_VaccinationModel/fit_with_vaccination/plots/Figure 8.pdf", 
