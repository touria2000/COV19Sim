functions {
  real[] SEIQHRDV(real t, real[] y, real[] theta, real[] x_r, int[] x_i) {
      ...
      real eta = theta[12];
      real theta_V = theta[13];

      // Force of infection
      real N = S + E + IA + IS + IQ + IH + R + D + V; 
      real lmbda  = (beta_A * IA + beta_S * IS + m_Q*beta_S*IQ + m_H*beta_S*IH) / N; 
      
      // ODE system
      real dS_dt = omega_R*R + omega_V*V - (lmbda + theta_V)*S;
      real dE_dt = lmbda*S - (1 - xi_V)*lmbda*V - alpha*E;
      ...
      real dV_dt = theta_V*S - (omega_V + (1 - xi_V)*lmbda)*V;
      
      return {dS_dt, dE_dt, dIA_dt, dIS_dt, dIQ_dt, dIH_dt, dR_dt, dD_dt, dV_dt};
  }
}
...
parameters {
  ...
  // Vaccination rate
  real<lower=0.002, upper=0.01> theta_V;
...
transformed parameters{
...

  {
    ...
    y_hat = integrate_ode_rk45(SEIQHRDV, y_init, t0, ts, theta, x_r, x_i);
  }
