...
model {
  ...
  gamma_S ~ normal(0.14 , 0.1);   // a mean of 1. / 7
  gamma_A ~ normal(0.14 , 0.1);   // a mean of 1. / 7
  gamma_H ~ normal(0.071 , 0.01); // a mean of 1. / 14
  gamma_Q ~ normal(0.071 , 0.01); // a mean of 1. / 14 
...
generated quantities {
  ...
  vector[n_pred] cases_pred;
  vector[n_pred] incidence_pred;
  
  // New initial conditions
  real y_init_pred[n_difeq] = y_hat[n_days, ]; 
  
  // New time zero is the last observed time
  real t0_pred = ts[n_days];
  
  real y_hat_pred[n_pred, n_difeq];
  
  {
    ...
    theta[12] = eta;
     ...
     y_hat_pred = integrate_ode_rk45(SEIQHRD, 
                                  y_init_pred, 
                                  t0_pred, 
                                  ts_pred, 
                                  theta, 
  ...
  for (i in 1:n_pred-1) {
    incidence_pred[i] = (1-rho)*alpha* y_hat_pred[i,2];
    cases_pred[i] = neg_binomial_2_rng(incidence_pred[i], phi);
    }
  }
}
